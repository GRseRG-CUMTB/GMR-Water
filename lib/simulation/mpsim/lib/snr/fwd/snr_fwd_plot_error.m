function snr_fwd_plot_error (result, max_plot, myplot)
  ...
  subplot(3,1,1)
    hold on
    myplot(result.sat.elev, result.snr_db, '-k', 'LineWidth',0.5)
  ...
  subplot(3,1,2)
    hold on
    myplot(result.sat.elev, result.carrier_error*1e3, '-k', 'LineWidth',0.5)
    myxlim(result.sat.elev([1 end]))
    set(gca(), 'XTickLabel',[])
    %xlabel('Elevation angle (degrees)')
    ylabel('Phase (mm)')
    grid on
  subplot(3,1,3)
    myplot(result.sat.elev, result.code_error*1e2, '-k', 'LineWidth',0.5)
    myxlim(result.sat.elev([1 end]))
    xlabel('Elevation angle (degrees)')
    ylabel('Code (cm)')
    grid on
